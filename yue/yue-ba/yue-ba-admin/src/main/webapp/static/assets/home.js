...
var languages = {
	...
	signup = {
		...
		check: function(a) {
			...
			"user_name" === a && (0 == c.length ? signup.check_fail(a) : !e.test(c) && 3 < c.length ? ($text(d, text.checking + "..."), $style.set(b, "backgroundImage", ""), $ajax(rootURL + "/api/signup.check.php", {
				data: {
					type: "user_name",
					user_name: c
				},
				success: function(b) {
					b ? ($text(d, text["name-ok"]), signup.check_suc(a)) : ($text(d, text["name-used"]), signup.check_fail(a))
				}
			...
			"email" === a && (0 == c.length ? signup.check_fail(a) : f.test(c) ? ($text(d, text.checking + "..."), $ajax(rootURL + "/api/signup.check.php", {
				data: {
					type: "email",
					email: c
				},
		...
		submit: function() {
			...
			signup.check("email");
			$data.get($("signup_nick_name"), "checked") && $data.get($("signup_user_name"), "checked") && $data.get($("signup_password"), "checked") && $data.get($("signup_email"), "checked") && ($className.add(a, "loading"), $text(a, text.signup_loading), $ajax(rootURL + "/api/signup.php", {
				data: {
					nick_name: $("signup_nick_name").value,
					user_name: $("signup_user_name").value,
				...
				success: function(b) {
					...
					b && ($cookie.set({
						...
						"new": "Y"
					}, 360), $cookie.set("is_registered", "true", .04), location.href = rootURL + "/welcome")
				}
			}))
		}
	};